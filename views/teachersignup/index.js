...
exports.init = function(req, res){
  if (req.isAuthenticated()) {
    ...
    res.render('teachersignup/index', {
      oauthMessage: '',
...
exports.teachersignup = function(req, res){
  ...
  workflow.on('createUser', function() {
    req.app.db.models.User.encryptPassword(req.body.password, function(err, hash) {
      ...
      req.app.db.models.User.create(fieldsToSet, function(err, user) {
        ...
        workflow.user = user;
        workflow.emit('createTeacher');
      });
    });
  });

  workflow.on('createTeacher', function() {
    var fieldsToSet = {
      user: {
    ...
    req.app.db.models.Teacher.create(fieldsToSet, function(err, teacher) {
      if (err) {
        return workflow.emit('exception', err);
      }

      //update user with teacher
      workflow.user.roles.teacher = teacher._id;
  ...
  workflow.on('sendWelcomeEmail', function() {
    req.app.utility.sendmail(req, res, {
      ...
      to: req.body.email,
      subject: 'Your '+ req.app.config.projectName +' Teacher',
      textPath: 'teachersignup/email-text',
      htmlPath: 'teachersignup/email-html',
      locals: {
        username: req.body.username,
...
exports.teachersignupTwitter = function(req, res, next) {
  req._passport.instance.authenticate('twitter', function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/teachersignup/');
    }
    ...
    req.app.db.models.User.findOne({ 'twitter.id': info.profile.id }, function(err, user) {
      ...
      if (!user) {
        ...
        res.render('teachersignup/social', { email: '' });
      }
      else {
        res.render('teachersignup/index', {
...
exports.teachersignupGitHub = function(req, res, next) {
  req._passport.instance.authenticate('github', function(err, user, info) {
    ...
    req.app.db.models.User.findOne({ 'github.id': info.profile.id }, function(err, user) {
      ...
      if (!user) {
        req.session.socialProfile = info.profile;
        res.render('teachersignup/social', { email: info.profile.emails && info.profile.emails[0].value || '' });
      }
      else {
        res.render('teachersignup/index', {
...
exports.teachersignupFacebook = function(req, res, next) {
  req._passport.instance.authenticate('facebook', { callbackURL: '/teachersignup/facebook/callback/' }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/teachersignup/');
    }
...
exports.teachersignupGoogle = function(req, res, next) {
  req._passport.instance.authenticate('google', { callbackURL: '/teachersignup/google/callback/' }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/teachersignup/');
    }
...
exports.teachersignupTumblr = function(req, res, next) {
  req._passport.instance.authenticate('tumblr', { callbackURL: '/teachersignup/tumblr/callback/' }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/teachersignup/');
    }
...
exports.teachersignupSocial = function(req, res){
  var workflow = req.app.utility.workflow(req, res);
  ...
  workflow.on('createTeacher', function() {
    ...
    req.app.db.models.Student.create(fieldsToSet, function(err, teacher) {
      if (err) {
        return workflow.emit('exception', err);
      }
